...
def build() -> Path:
    ...
    color = {"ALERT": ALERT, "WATCH": WATCH, "OK": OK}
    rows = "".join(
        f'<tr><td>{r.station_id}</td><td>{r.line_id}</td><td>{r.n_first_pass:,}</td>'
...
<style>
  ...
  th{{background:{ACCENT};color:#fff;font-weight:600}} td:first-child{{font-weight:700}}
  ...
  <table><tr><th>Station</th><th>Line</th><th>First-pass n</th><th>FPY</th>
